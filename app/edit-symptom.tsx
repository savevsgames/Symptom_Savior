...
export default function EditSymptom() {
  ...
  const { getSymptomById, updateSymptom } = useSymptoms();
  ...
  const loadSymptom = async () => {
    try {
      ...
      if (data) {
        ...
        if (commonSymptoms.includes(data.symptom)) {
          setSelectedSymptom(data.symptom);
          setCustomSymptom('');
        } else {
          setSelectedSymptom('');
          setCustomSymptom(data.symptom);
        }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const { error } = await updateSymptom(id, updateData);
      ...
      Alert.alert(
        'Symptom Updated',
        `Your ${symptomName.toLowerCase()} has been updated successfully.`,
    ...
    return (
      <SafeAreaView style={styles.container}>
        ...
        <View style={styles.errorContainer}>
          <Text style={styles.errorText}>Symptom not found or you don't have permission to edit it.</Text>
          <BaseButton
            title="Go Back"
            onPress={() => router.back()}
            variant="primary"
          ...
          <View style={styles.symptomGrid}>
            {commonSymptoms.map((symptomOption) => (
              <TouchableOpacity
                ...
                style={[
                  styles.symptomChip,
                  selectedSymptom === symptomOption && styles.symptomChipSelected
                ]}
                onPress={() => {
                  setSelectedSymptom(symptomOption);
                  setCustomSymptom('');
                }}
              >
                <Text style={[
                  styles.symptomChipText,
                  selectedSymptom === symptomOption && styles.symptomChipTextSelected
                ]}>
                  {symptomOption}
                </Text>
              </TouchableOpacity>
